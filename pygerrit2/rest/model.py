...
class GerritChange(object):
    def __init__(self, gerrit_client, **data):
    ...
    def rebase(self):
        return self.gerrit_client.post('/changes/%s/rebase' % self.id)

    def abandon(self):
        return self.gerrit_client.post('/changes/%s/abandon' % self.id)

    def get_topic(self):
        return self.gerrit_client.get('/changes/%s/topic' % self.id)

    def set_topic(self, topic):
        return self.gerrit_client.put('/changes/%s/topic' % self.id, json={'topic': topic})

    def delete_topic(self):
        return self.gerrit_client.delete('/changes/%s/topic' % self.id)

    def submit(self):
        return self.gerrit_client.post('/changes/%s/submit' % self.id)

    def get_file_content(self, filename, revision='current'):
        return self.gerrit_client.get('/changes/%s/revisions/%s/files/%s/content' % (self.id, revision, quote_plus(filename)))
    ...
    def change_file_content_in_edit(self, filename, stream):
        return self.gerrit_client.put('/changes/%s/edit/%s' % (self.id, quote_plus(filename)), data=stream)

    def delete_file_in_edit(self, filename):
        return self.gerrit_client.delete('/changes/%s/edit/%s' % (self.id, quote_plus(filename)))

    def publish_edit(self):
        return self.gerrit_client.post('/changes/%s/edit:publish' % self.id)

    def delete_edit(self):
        return self.gerrit_client.delete('/changes/%s/edit' % self.id)

    def publish(self):
        return self.gerrit_client.post('/changes/%s/publish' % self.id)

    def add_reviewer(self, reviewer):
        return self.gerrit_client.post('/changes/%s/reviewers' % self.id, json={'reviewer': reviewer})
    ...
    def add_review(self, **kwargs):
        ...
        """
        self.reload()
        return self.gerrit_client.post('/changes/%s/revisions/%s/review' % (self.id, self.revision), json=kwargs)

    def reload(self):
        self.__dict__.update(self.gerrit_client.get_change(change=self.id).__dict__)
